...
sources:
  ...
  - 'AD-9 añadido 2026-08-17: npm run build falló con "Server Actions are not supported with static export" en main tras mergear PR github-pages + PR de remediación Epic 1-3'
companions: []
---
...
### AD-9 — Target de deployment incompatible con static export [añadido 2026-08-17]

- **Binds:** next.config.ts, app/**/actions.ts, .github/workflows/
- **Prevents:** que un futuro cambio de hosting reintroduzca `output: "export"` y rompa el build en silencio
- **Rule:** `next.config.ts` nunca establece `output: "export"`. Los Server Actions (`'use server'` en `app/**/actions.ts`, base de AD-1 a AD-4) son incompatibles con el modo de export estático de Next.js — un intento de build falla con `Server Actions are not supported with static export`. El deployment requiere un host con runtime Node.js (Vercel, Netlify, o equivalente); GitHub Pages (solo estático) no es una opción viable mientras AD-1 a AD-4 sigan vigentes.
- **Hallado:** un PR de configuración de GitHub Pages (`output: "export"`) y el trabajo de remediación de Epic 1-3 (Server Actions) se mergearon a `main` de forma independiente sin detectar el conflicto — `npm run build` estuvo roto en `main` hasta que se detectó y corrigió.
